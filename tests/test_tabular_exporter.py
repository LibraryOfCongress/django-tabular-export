...
def assert_is_valid_xlsx(bytestream, required_filename='xl/worksheets/sheet1.xml'):
    ...
    if required_filename not in zip_filenames:
        raise AssertionError('Expected to find %s in %s' % (required_filename, zip_filenames))
...
class SimpleUtilityTests(unittest.TestCase):
    ...
    def test_convert_value_to_unicode(self):
        ...
        self.assertEqual(b'\xc3\x9cnic\xc3\xb0e', convert_value_to_unicode(u'Ünicðe').encode('utf-8'))
        self.assertEqual('2015-08-28T00:00:00',
                         convert_value_to_unicode(datetime.datetime(year=2015, month=8, day=28)))
        self.assertEqual('2015-08-28',
    ...
    def test_set_content_disposition(self):
        ...
        self.assertEqual({'Content-Disposition': "attachment; filename*=UTF-8''not%20a%20real%20file"},
    ...
    def test_ensure_filename(self):
        ...
        def fake_admin_action(modeladmin, request, queryset, filename=None, *args, **kwargs):
            return filename

        fake_ma = FakeModelAdmin()

        # Confirm that the auto-generated filename
        self.assertEqual('test models.test', fake_admin_action(fake_ma, None, None),
                         msg="Standard filenames should be the model's verbose_name_plural with the "
                             "provided extension")
        self.assertEqual('custom', fake_admin_action(fake_ma, None, None, filename='custom'),
...
class QuerySetTests(TestCase):
    ...
    def test_flatten_queryset(self):
        ...
        self.assertListEqual(['The Title'], headers, msg='extra_verbose_names must override default headers')
        self.assertListEqual(list(rows), [('ABC', )])
...
class ResponseTests(SimpleTestCase):
    ...
    def get_test_data(self):
        ...
        return ['Foo Column', 'Bar Column'], ((1, 2), (3, 4), ('abc', 'def'),
    ...
    def test_export_to_excel_response(self):
        ...
        self.assertEqual('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                         resp['Content-Type'])
        self.assertEqual("attachment; filename*=UTF-8''test.xlsx", resp['Content-Disposition'])
    ...
    def test_export_csv_using_generator(self):
        ...
        self.assertEqual("attachment; filename*=UTF-8''numbers.csv", resp['Content-Disposition'])
    ...
    def test_export_excel_using_generator(self):
        ...
        self.assertEqual("attachment; filename*=UTF-8''numbers.xlsx", resp['Content-Disposition'])
